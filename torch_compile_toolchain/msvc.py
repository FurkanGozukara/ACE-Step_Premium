...
def load_msvc_environment(env: MutableMapping[str, str]) -> MsvcLoadStatus:
    ...
    if failures:
        detail = (
            "No usable CUDA-compatible MSVC toolset found. Tried: "
...
def _candidate_msvc_scripts(
    ...
    return _sort_msvc_candidates(_existing_unique_scripts(candidates))
...
def visual_studio_install_roots(
    ...
    for env_key in ("VSINSTALLDIR", "VCINSTALLDIR", "VCToolsInstallDir"):
...
def _vswhere_path(env: Mapping[str, str] | None = None) -> Path | None:
    source_env = os.environ if env is None else env
    configured = _environment_value(source_env, "VSWHERE")
...
def _environment_from_vc_script(
    ...
    *,
    ...
    if completed.returncode != 0:
        error = _last_nonempty_line(_decode_cmd_output(completed.stderr)) or (
...
def _msvc_toolset_versions(root: Path) -> list[str]:
    ...
    versions = [
        ...
        if _MSVC_TOOLSET_PATTERN.fullmatch(path.name)
        and _toolset_has_x64_cl(path)
    ]
...
def _toolset_has_x64_cl(toolset: Path) -> bool:
    return any(
        (toolset / "bin" / host / "x64" / "cl.exe").is_file()
            ...
            for version in versions:
                installed_editions = sorted(
                    (
                        ...
                        if path.is_dir()
                        and ((path / "Common7").is_dir() or (path / "VC").is_dir())
                    ),
                    key=lambda path: (
...
def _find_visual_studio_root(path: Path) -> Path | None:
    ...
    for candidate in candidates:
        if (candidate / "VC" / "Auxiliary" / "Build").is_dir() or (
            candidate / "Common7" / "Tools"
        ).is_dir():
            return candidate
