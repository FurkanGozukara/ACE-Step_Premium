...
def _cuda_root_candidates(env: MutableMapping[str, str], platform_name: str) -> list[Path]:
    ...
    if conda_prefix:
        ...
        candidates.extend(
            conda_root / "targets" / target
            for target in ("x86_64-linux", "sbsa-linux", "aarch64-linux")
        )
    ...
    if platform_name == "win32":
        ...
        for value in (item for item in program_files if item):
            default_root = _path_from_environment(value) / (
...
def _looks_like_cuda_root(path: Path, platform_name: str) -> bool:
    if (path / "bin" / _exe_name("nvcc", platform_name)).is_file() or (
        path / "include" / "cuda.h"
    ).is_file():
        return True
    library_patterns = (
