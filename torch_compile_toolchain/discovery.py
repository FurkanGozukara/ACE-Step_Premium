...
def discover_posix_compiler(env: MutableMapping[str, str]) -> ExecutableStatus:
    ...
    for env_key in ("NVCC_CCBIN", "CUDAHOSTCXX", "CXX"):
        ...
        cxx_path = (
            _resolve_compiler_executable(configured_cxx, env) if configured_cxx else ""
        )
        if not cxx_path:
            if configured_cxx:
        ...
        if probe.ok:
            ...
            return ExecutableStatus(
                True,
                f"{env_key} compiler found: {cxx_path}; {probe.detail}",
                cxx_path,
            )
...
def _posix_compiler_candidates(
    ...
    versioned_clang = [
        (f"clang++-{version}", f"clang-{version}") for version in range(30, 3, -1)
    ]
    candidates = (
        ("g++", "gcc"),
        *discovered,
    ...
    *,
    ...
    if not prioritize:
        ...
        additions = [
            ...
            if _normalized_path_key(Path(part)) not in existing_keys
        ]
        if additions:
    ...
    remaining = [
        ...
        if _normalized_path_key(Path(part)) not in preferred_keys
    ]
