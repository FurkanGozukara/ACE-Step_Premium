...
def _ensure_compile_cache_dirs(
    ...
    root_text = str(
        project_root
        or env.get("TORCH_COMPILE_PROJECT_ROOT")
        or ""
    ).strip()
    ...
    preferred = (
        ...
        else root / ".cache" / "torch_compile_toolchain"
    )
    fallback = Path(tempfile.gettempdir()) / "torch_compile_toolchain"
    for cache_root in _unique_cache_roots(preferred, fallback):
        inductor = (
            Path(configured_inductor).expanduser()
            if configured_inductor
            else cache_root / "inductor"
        )
        triton = (
            Path(configured_triton).expanduser()
            if configured_triton
            else cache_root / "triton"
        )
        try:
...
def _requirements_satisfied(
    ...
    *,
    ...
    return (
        compiler_ok
        and (cuda_ok or not require_cuda_toolkit)
        and (ninja_ok or not require_ninja)
    )
