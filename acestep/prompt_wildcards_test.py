...
class PromptWildcardTest(unittest.TestCase):
    ...
    def test_literal_brackets_can_wrap_nested_wildcards(self):
        self.assertEqual(
            "tag [keep a]",
            expand_prompt_wildcards("tag [keep [a|b]]", rng=_FirstChoice()),
        )

    def test_missing_closing_bracket_reports_syntax_error(self):
        with self.assertRaisesRegex(WildcardSyntaxError, "Missing closing"):
            expand_prompt_wildcards("modern [warm|bright")

    def test_unexpected_closing_bracket_reports_syntax_error(self):
