...
def apply_fp8_scaled_quantization(
    ...
    *,
    ...
    Args:
        ...
        skip_trainable: Leave trainable Linear layers untouched for LoRA training.
    """
    ...
    patched = _apply_entries_to_model(model, entries, skip_trainable=skip_trainable)
...
def _load_or_build_cache(
    *,
    ...
    cache_file = _cache_file_for_checkpoint(checkpoint_path, skip_trainable=skip_trainable)
    cached_entries = load_cached_entries(cache_file)
    if cached_entries is not None:
        return cached_entries

    entries = _build_fp8_entries(model, device, skip_trainable=skip_trainable)
...
def _build_fp8_entries(
    ...
    for name, module in _iter_target_linears(model):
        if skip_trainable and module.weight.requires_grad:
            continue
...
def _cache_file_for_checkpoint(
    checkpoint_path: str | os.PathLike[str] | None,
    *,
    skip_trainable: bool = False,
) -> Path:
    ...
    variant = "lora_skip_trainable" if skip_trainable else ""
    return cache_file_for_checkpoint(checkpoint_path, variant=variant)
