"""CUDA host compiler compatibility checks for ``torch.compile`` setup."""
...
def probe_cuda_compiler(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="acestep_cuda_probe_") as temp_dir:
        temp_path = Path(temp_dir)
        source = temp_path / "probe.cu"
        output = temp_path / ("probe.obj" if platform_name == "win32" else "probe.o")
        source.write_text('extern "C" __global__ void acestep_probe() {}\n', encoding="utf-8")
