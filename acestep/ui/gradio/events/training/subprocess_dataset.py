...
_PREPROCESS_STOPPED_STATUS = "Isolated tensor preprocess subprocess stopped."
    ...
    try:
        for event in stream_training_subprocess_job(payload, job):
            if event.get("kind") == "status":
                ...
                return _preprocess_status_from_result(event["result"], status)
    except Exception as exc:
...
def _preprocess_status_from_result(result: dict[str, Any], fallback: str) -> str:
    """Return tensor-preprocess wording for a subprocess result status."""

    status = str(result.get("status") or fallback)
    if status == _GENERIC_STOPPED_STATUS:
        return _PREPROCESS_STOPPED_STATUS
    return status
