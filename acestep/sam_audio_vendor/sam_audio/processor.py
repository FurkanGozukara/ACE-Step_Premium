    ...
    for size, video in zip(sizes, videos, strict=False):
        ...
        if isinstance(video, str):
            if VideoDecoder is None:
                raise RuntimeError("torchcodec video decoding is unavailable.")
...
class Processor:
    ...
    def mask_videos(
        ...
        if VideoDecoder is None and (
            any(isinstance(v, str) for v in videos)
            or any(isinstance(m, str) for m in masks)
        ):
            raise RuntimeError("torchcodec video decoding is unavailable.")
...
class SAMAudioJudgeProcessor(Processor):
    ...
    def _load_audio(self, path: str):
        if AudioDecoder is None:
            raise RuntimeError("torchcodec audio decoding is unavailable.")
